...
export default function Display() {
  ...
  return <section className="display-screen fade-in">
    <div className="display-top"><div><strong>عيادات المشفى</strong><span>{selectedDoctor?.name} - {selectedDoctor?.room}</span></div><div>{time.toLocaleTimeString('ar-EG', { hour: '2-digit', minute: '2-digit' })}</div></div>
    ...
    <div className="upcoming-strip"><span>الأدوار القادمة</span><div>{upcoming.map(b => <b key={b.id}>{b.queueNumber}</b>)}</div></div>
  </section>
}
