...
export default function AddBookingForm() {
  ...
  useEffect(() => {
    if (!form.doctorId && activeDoctors[0]?.id) setForm(v => ({ ...v, doctorId: activeDoctors[0].id }))
  ...
  return <>
    <button className="primary-btn" onClick={() => setOpen(true)} disabled={activeDoctors.length === 0}><PlusCircle size={18}/> حجز جديد</button>
    {open && <div className="modal-backdrop" onClick={() => setOpen(false)}>
      <form className="modal-card" onClick={e => e.stopPropagation()} onSubmit={submit}>
        <h2>إضافة حجز جديد</h2>
        {activeDoctors.length === 0 && <p className="form-message danger-text">أضف طبيبًا أولًا قبل تسجيل الحجوزات.</p>}
        ...
        <label>الطبيب<select value={form.doctorId} onChange={e => setForm({ ...form, doctorId: e.target.value })}>{activeDoctors.map(d => <option key={d.id} value={d.id}>{d.name} - {d.room} - كود {d.codePrefix}</option>)}</select></label>
        ...
        <div className="modal-actions"><button type="button" className="ghost-btn" onClick={() => setOpen(false)}>إلغاء</button><button className="primary-btn" disabled={activeDoctors.length === 0}>حفظ وإنشاء رقم دور</button></div>
      </form>
    </div>}
  </>
}
